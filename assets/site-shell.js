...
function start(){
  ...
  // Remove any accidental duplicate nav from previous versions.
  document.querySelectorAll("#hlrn-global-nav").forEach((el,i)=>{
    if(i>0) el.remove();
  });
  if(document.getElementById("hlrn-global-nav")) return;

  const scripts=[...document.scripts];
  const shellScript=scripts.slice().reverse().find(s=>
    /(?:^|\/)site-shell\.js(?:\?|$)/i.test(s.src||"")
  );

  let root;
  try{
    if(shellScript && shellScript.src){
      root=new URL("../",shellScript.src);
    }
  }catch(e){}

  if(!root){
    const marker="/HLRN-Website/";
    const pathname=location.pathname||"/";
    const lower=pathname.toLowerCase();
    const idx=lower.indexOf(marker.toLowerCase());
    root=new URL(
      idx>=0 ? pathname.slice(0,idx+marker.length) : "/",
      location.origin
    );
  }
  ...
  const primaryLinks=primary.map(([key,label,target])=>
    `<a class="hgn-link ${active===key?"active":""}" href="${url(target)}">${label}</a>`
  ).join("");

  const moreLinks=more.map(([key,label,target])=>
    `<a class="hgn-more-item ${active===key?"active":""}" href="${url(target)}">${label}</a>`
  ).join("");

  const mobilePrimary=primary.map(([key,label,target])=>
    `<a class="${active===key?"active":""}" href="${url(target)}">${label}</a>`
  ).join("");

  const mobileMore=more.map(([key,label,target])=>
    `<a class="${active===key?"active":""}" href="${url(target)}">${label}</a>`
  ).join("");

  nav.innerHTML=`
    <div class="hgn-inner">
      <a class="hgn-brand" href="${url("")}">
      ...
      <div class="hgn-links">
        ...
        <div class="hgn-more-wrap ${more.some(x=>x[0]===active)?"active":""}">
          <button class="hgn-link hgn-more-btn" type="button" aria-expanded="false">
            MORE <span class="hgn-arrow">▾</span>
          </button>
          <div class="hgn-more-menu">
            ${moreLinks}
          </div>
        </div>
      </div>

      <a class="hgn-live" href="${url("live/")}">
        <i></i> RACE CENTER
      </a>
  ...
  // Redirect any old Schedule links to Results.
  document.addEventListener("click",e=>{
    ...
    if(
      ...
      u.pathname.toLowerCase().includes("/hlrn-website/schedule/")
    ){
      e.preventDefault();
    ...
    if(
      u.hostname.toLowerCase()==="sites.google.com" &&
      u.pathname.toLowerCase().includes("/view/highlineracingnetwork")
    ){
      const p=u.pathname.toLowerCase();
      let dest="";
      if(p.includes("race-intelligence")) dest="race-intelligence/";
      ...
      else if(p.includes("driver")||p.includes("meet-our-team")) dest="drivers/";
